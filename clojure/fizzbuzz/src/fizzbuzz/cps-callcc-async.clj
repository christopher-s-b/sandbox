...
(defn mk-cps
  ...
  (call-cc (fn [c-resume] ;`c-resume` is a "callback" to the business logic
             (let [inner-callback (fn [response] ;build an internal callback for the query function
               ...
               (f-async inner-callback) ;execute the query, it will resume the business logic via above
